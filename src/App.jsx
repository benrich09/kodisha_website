...
const howRenter = [
  ...
  { num: '02', title: 'Browse and search', desc: 'Explore categories, filter by location, dates and price. View photos, rules and owner ratings.' },
  { num: '03', title: 'Book and pay securely', desc: 'Request or instant-book. Pay in-app with optional deposit protection. Funds are held until the rental is confirmed.' },
  { num: '04', title: 'Handover and use', desc: 'Meet the owner or arrange delivery. Confirm item condition with photos, then enjoy the rental period.' },
  { num: '05', title: 'Return and review', desc: 'Return on time. Leave a review. Deposit is released after a successful return. Disputes use photo evidence.' },
]
...
const howOwner = [
  ...
  { num: '05', title: 'Get paid and review', desc: 'After return, payout is released to you. Rate the renter and build your reputation for more bookings.' },
]

function StoreButtons({ light, business }) {
  ...
  return (
    <section className="relative min-h-screen overflow-hidden">
      ...
      <div className="relative mx-auto flex min-h-screen max-w-6xl flex-col justify-center px-4 pb-16 pt-28 sm:px-6">
        {title && <p className="text-xs font-semibold uppercase tracking-[0.2em] text-teal-300">{title}</p>}
        {subtitle && <h1 className="mt-3 max-w-2xl text-3xl font-bold text-white sm:text-4xl lg:text-5xl">{subtitle}</h1>}
        {children}
      </div>
    </section>
  )
    ...
    <button
      ...
      className={`block w-full rounded-lg px-4 py-3 text-left text-sm font-medium transition md:inline-block md:w-auto md:px-3 md:py-2 ${
        page === id
          ...
          : solid ? 'text-slate-700 hover:bg-teal-50/60 hover:text-teal-700' : 'text-white/90 hover:bg-white/15 hover:text-white'
      }`}
    >
      {label}
    </button>
      ...
      <div className="mx-auto max-w-6xl px-4 sm:px-6">
            ...
            <div className="relative ml-2">
              ...
              {regOpen && (
                <div className="absolute right-0 mt-2 w-64 overflow-hidden rounded-2xl border border-slate-100 bg-white py-2 shadow-xl">
                  ...
                  <button onClick={() => { setPage('get-app'); setRegOpen(false) }} className="flex w-full items-center gap-3 px-4 py-3.5 text-left text-sm hover:bg-teal-50">
                    <span className="flex h-9 w-9 items-center justify-center rounded-xl bg-slate-100 text-lg">📱</span>
                    <div><p className="font-semibold text-slate-900">Get the app</p><p className="text-xs text-slate-500">Customer or business</p></div>
                  </button>
                </div>
              )}
            </div>
        ...
        {open && (
          <div className="md:hidden">
            <div className={`flex flex-col border-t py-3 pb-5 rounded-b-2xl backdrop-blur-xl ${
              ...
              <div className="mx-3 mt-2 space-y-2 border-t border-slate-200/30 pt-3">
                ...
                <button onClick={() => { setPage('get-app'); setOpen(false) }} className="w-full rounded-xl border border-teal-300 py-3 text-sm font-semibold text-teal-100 bg-teal-500/20">Get the app</button>
              </div>
            </div>
          </div>
        )}
    ...
    <>
        ...
        <div className="relative mx-auto flex min-h-screen max-w-6xl flex-col justify-center px-4 pb-16 pt-28 sm:px-6">
          <div className="max-w-2xl">
            ...
            <p className="mt-5 max-w-lg text-base leading-relaxed text-slate-200 sm:text-lg">
              Kodisha is the trusted marketplace where owners list apartments, cars, cameras, tools, fashion and more. Renters book, pay and return safely.
            </p>
      ...
      <section className="bg-teal-700 text-white">
        <div className="mx-auto grid max-w-6xl grid-cols-2 gap-6 px-4 py-9 sm:grid-cols-4 sm:px-6">
          {[{ value: '8+', label: 'Categories' }, { value: 'Secure', label: 'Payments and deposits' }, { value: '24/7', label: 'Active rental support' }, { value: 'TZ', label: 'Built for Tanzania' }].map((s) => (
      ...
      <section className="border-t border-slate-100 bg-slate-50 py-16 sm:py-20">
        <div className="mx-auto max-w-6xl px-4 sm:px-6">
          <div className="mb-10 text-center">
            ...
            <h2 className="mt-2 text-2xl font-bold text-slate-900 sm:text-3xl">For renters and owners</h2>
          </div>
...
function GetAppPage() {
  ...
  return (
    <PageHero title="Get the app" subtitle="Choose how you use Kodisha">
      {!role ? (
        <div className="mt-10 grid max-w-2xl gap-4 sm:grid-cols-2">
          ...
          >
            ...
            <h3 className="mt-3 text-xl font-bold text-white">Customer / Renter</h3>
            <p className="mt-2 text-sm text-slate-200">Find and book apartments, cars, tools, fashion and more.</p>
          </button>
          <button
          ...
          >
            <span className="text-3xl">📦</span>
            <h3 className="mt-3 text-xl font-bold text-white">Business / Owner</h3>
            <p className="mt-2 text-sm text-slate-200">List your items, manage bookings and earn from what you own.</p>
          </button>
        </div>
      ) : (
        <div className="mt-10 max-w-lg">
          <button onClick={() => setRole(null)} className="mb-4 text-sm font-medium text-teal-200 hover:text-white">← Back</button>
          <h2 className="text-2xl font-bold text-white">
            {role === 'customer' ? 'Customer app' : 'Business app'}
          </h2>
          <p className="mt-2 text-slate-200">
            {role === 'customer'
              ? 'Download the Kodisha app for renters. Free on Google Play and the App Store.'
              : 'Download the Kodisha Business app for owners. Free on Google Play and the App Store.'}
          </p>
          <div className="mt-8">
...
function Services() {
  return (
    <div>
      <PageHero title="Discover" subtitle="What you can rent on Kodisha">
...
function HowItWorksPage() {
  return (
    <div>
      <PageHero title="Process" subtitle="How Kodisha works">
        <p className="mt-4 max-w-lg text-slate-200">Clear steps for renters and owners from signup to completed rental.</p>
      </PageHero>
...
function About() {
  return (
    <div>
      <PageHero title="Our story" subtitle="About Kodisha">
        <p className="mt-4 max-w-lg text-slate-200">Built so people can rent almost anything safely and owners can earn from underused assets.</p>
      </PageHero>
      <div className="mx-auto max-w-3xl px-4 py-14 sm:px-6 sm:py-20">
        <div className="space-y-8">
          {[
            ...
            { title: 'Trust and confidentiality', body: 'We verify users with identity documents. Payments and deposits are handled securely. Personal documents are used only for verification and are not shared with other users beyond what is needed for a safe rental.' },
          ].map((s) => (
...
function Contact() {
  return (
    <div>
      <PageHero title="Support" subtitle="Contact and Support">
        <p className="mt-4 max-w-lg text-slate-200">We are here to help renters and owners every step of the way.</p>
      </PageHero>
...
function RegisterForm({ role }) {
  ...
  if (done) {
    return (
      <PageHero title="Almost ready" subtitle="Log in on the Kodisha app">
        <div className="mt-6 max-w-md">
          <img src="/logo.png" alt="Kodisha" className="h-16 w-16 object-contain brightness-0 invert" />
          <p className="mt-4 text-slate-200">
            Your {isRenter ? 'renter' : 'owner'} registration has been received. For security, complete verification and log in inside the Kodisha mobile app with the phone number and password you just used.
          </p>
          <div className="mt-8"><StoreButtons light business={!isRenter} /></div>
        </div>
      </PageHero>
    )
  }
  ...
  return (
    <div>
      <PageHero title={isRenter ? 'Renter' : 'Owner'} subtitle={`Register as ${isRenter ? 'Renter' : 'Owner'}`}>
        <p className="mt-3 text-slate-200">{isRenter ? 'Find and book items near you.' : 'List your items and start earning.'}</p>
      </PageHero>
      <div className="mx-auto max-w-md px-4 py-14 sm:px-6 sm:py-16">
        ...
        <form className="space-y-3.5" onSubmit={(e) => { e.preventDefault(); setDone(true) }}>
          ...
          <div className="rounded-xl border border-slate-200 bg-slate-50 p-4">
            <p className="text-xs font-semibold text-slate-700">Identity document (required for confidentiality and trust)</p>
...
function Footer({ setPage }) {
  return (
    <footer className="mt-auto bg-gradient-to-br from-teal-700 via-teal-800 to-teal-900 text-teal-100/90">
      <div className="mx-auto max-w-6xl px-4 py-14 sm:px-6">
        <div className="mb-10 text-center">
          <img src="/logo.png" alt="Kodisha" className="mx-auto mb-4 h-16 w-16 object-contain brightness-0 invert sm:h-20 sm:w-20" />
          <h2 className="text-xl font-bold text-white sm:text-2xl">Download the Kodisha app</h2>
          <p className="mt-2 text-sm text-teal-100">Google Play and App Store free for renters and owners</p>
          <div className="mt-6 flex justify-center"><StoreButtons light /></div>
        </div>
        <div className="mb-10 grid grid-cols-2 gap-10 border-t border-teal-600/40 pt-10 md:grid-cols-4">
          <div className="col-span-2 md:col-span-1">
            <div className="mb-3 flex items-center gap-3">
              <img src="/logo.png" alt="Kodisha" className="h-12 w-12 object-contain brightness-0 invert" />
              <span className="text-lg font-bold text-white">Kodisha</span>
            </div>
            <p className="text-xs leading-relaxed text-teal-200/80">The trusted platform to rent almost anything safely and conveniently.</p>
          ...
          <div>
            ...
            <ul className="space-y-2.5 text-xs">
              ...
              <li><button onClick={() => setPage('get-app')} className="hover:text-white">Get the app</button></li>
          ...
          <div>
            <h4 className="mb-4 text-sm font-semibold text-white">Stores</h4>
            <ul className="space-y-2.5 text-xs">
        ...
        <div className="flex flex-col items-center justify-between gap-4 border-t border-teal-600/40 pt-8 text-xs sm:flex-row">
...
export default function App() {
  ...
  const heroPages = ['home', 'services', 'how', 'about', 'contact', 'get-app', 'register-renter', 'register-owner']
